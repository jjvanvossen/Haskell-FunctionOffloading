...
### Tutorial (See the example files for reference!!)
...
This part of the guide describes how to use the thesis' implementation for manual offloading of annotated Haskell functions, which allows for more design freedom (including pipelining!) as opposed to the previous semi-automated offloading. Refer to the thesis and the code for more design and implementation detail. First the requirements are described and subsequently the tutorial is given. An example can be found in the folder _/Examples/Manual/_ and _/Examples/Manual_pipelined/_ of this repository, which can also be used as a template for custom projects.
...
+ All Haskell functions that are to be offloaded should adhere to the following requirements:
  ...
  + A composition of the previously mentioned offloadable functions and a higher-order data-flow function of CLaSH has to exist, i.e. the function can be passed as an argument to either the **_pureDF_**, **_mealyDF_**, or **_mooreDF_** function.
...
3. Create the top-entity module for the CLaSH compilation process of the hardware partition. A template file is available in this repository, which includes practical examples(read it!!!!!). Import the **_module set 2_** and correctly fill in the argument vector of offloadable functions of the **_topWrapper_** function in the template file. Also refer to the example files for more information. In addition to the given examples, it is also possible to combine certain functions using the CLaSH 'seqDF' sequential combinator function (see CLaSH dataflow for more information), which allows for offloading more complex multi-clockcycle functions. The resulting function will then be visible to the rest of the system design as a dataflow function with the first input type and last output type of the sequentially combined functions. Lastly it should be noted that a manual Signal-based CLasH function can be designed which inherently already adheres to the correct dataflow type, which can be subsequently lifted to the appropriate type using the CLaSH 'liftDF' function. This allows for even more complex multi-clockcycle offloadable functions, e.g. a custom soft-core processor.
...
6. The desired offloadable functions in the **_module set 2_** should now be manually replaced (it is recommended to make a copy of **_module set 2_** here to retain the old functions) with the **_offloadTemplate_** function or in the case of a pipelined design with the **_offloadWriteTemplate_** and **_offloadReadTemplate_** functions. This includes the correct assignment of **_function identifiers_** and **_serialisation naturals_** (See the template file of step 3 and examples on how to produce these values). The **_module 1_**, which calls upon the replaced offloadable functions, should now be modified accordingly. In the case of a pipelined utilization of the offloadable function, the **_module 1_** should be converted into a design with separate write and read threads. See the example files for details. In the cases of the sequentially combined offloaded functions (using 'seqDF') or custom lifted Signal based functions (using 'liftDF'), then the Haskell interfacing functions should be designed appropriately at this step.
...
## General tasks
This guide is written with a linux host pc in mind and with the specific SD-card image used in this thesis. Any reproductions of the SD-card will most likely have different configurations. In most cases the Xillybus IP core documents can also be used for information.
